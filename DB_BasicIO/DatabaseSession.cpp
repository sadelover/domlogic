...
bool CDatabaseSeesion::AddMysqlErrLog( string strOperation, string strErrInfo )
{
	...
	if(Project::Tools::FindOrCreateFolder(strFilePath))
	{
		...
		GetLocalTime(&stNow);
		CString strLogPath;
		strLogPath.Format(_T("%s\\domlogic_mysql_err_%d_%02d_%02d.log"),strFilePath.c_str(),stNow.wYear,stNow.wMonth,stNow.wDay);
		CStdioFile	ff;
...
bool CDatabaseSeesion::AddMysqlErrSqlLog( string strOperation, string strErrInfo, string strErrSql )
{
	...
	if(Project::Tools::FindOrCreateFolder(strFilePath))
	{
		...
		GetLocalTime(&stNow);
		CString strLogPath;
		strLogPath.Format(_T("%s\\domlogic_mysqlsql_err_%d_%02d_%02d.log"),strFilePath.c_str(),stNow.wYear,stNow.wMonth,stNow.wDay);
		CStdioFile	ff;
		if(ff.Open(strLogPath,CFile::modeCreate|CFile::modeNoTruncate|CFile::modeWrite))
		{
