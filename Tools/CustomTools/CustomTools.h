...
namespace Project
{
	namespace Tools
	{
		...
		inline void GetAppName(wstring &strAppName)
		{
			wchar_t szExePath[MAX_PATH] = {0};
			::GetModuleFileName(NULL, szExePath, MAX_PATH);
			...
			int nFindIndex = strSysPath.find_last_of('\\');
			strAppName = strSysPath.substr(nFindIndex+1);
		}

		inline bool StringContainElementOfVector(const wstring wstr, const vector<wstring> & vecWstr)
		{
			for(int i=0;i<vecWstr.size();i++)
			{
				int xfind = wstr.find(vecWstr[i].c_str());
				if(xfind>=0)
					return true;
			}
		...
		*/
		...
		inline void SysTime_To_DateString( const SYSTEMTIME & systime, tstring & wstrDate )
		{
			TCHAR aszTmp[256]	= {0};
		...
		*/
		...
		inline CString CombineStringList(std::vector<tstring> &strArray, wstring wstrSplit)
		{//split allStr to strArray
			// "3/4/5" "/" ->(3,4,5) array
			if(strArray.size()==0)
				return _T("");
			...
			{
				strAll+= strArray[i].c_str();
				if(i<(strArray.size()-1))
					strAll+= wstrSplit.c_str();
			}
		...
		inline BOOL FindProcessByName(CString exefile)
		{
			...
			if (Process32First(handle, &pInfo))
			{
				while (Process32Next(handle, &pInfo) != FALSE)
				{
					...
					if (0 == strTemp.CompareNoCase(exefile) || strTemp.Find(exefile)==0)
					{
