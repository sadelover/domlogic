...
#pragma warning(disable:4996 4018 4244 4267)

//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

IOCPS::IOCPS() : m_FileTransmitSuccHandle(NULL)
{
	...
	m_nOfWorkers=2;
	m_nPortNumber=999;
	m_sServerVersion=IOCP_SERVER_VERSION;
	m_NumberOfActiveConnections=0;
	m_bSendInOrder=FALSE;
	m_bReadInOrder=FALSE;
	m_iNumberOfPendlingReads=3;
	m_bIsFileTransmit = false;

	InitializeCriticalSection(&m_criFileTransmit);
	m_FileTransmitSuccHandle = CreateEvent(NULL, FALSE, FALSE, NULL);
}

IOCPS::~IOCPS()
{
	if (m_FileTransmitSuccHandle != NULL)
	{
		CloseHandle(m_FileTransmitSuccHandle);
		m_FileTransmitSuccHandle = NULL;
	}

	LeaveCriticalSection(&m_criFileTransmit);
...
BOOL IOCPS::Startup()
{
	...
	if ( !m_bServerStarted )
	{
		...
		*/
		...
		msg.Format("Maximum nr of simultaneous connections: %i.  \r\n",m_iMaxNumConnections);
		AppendLog(msg);


		msg.Format("Number of pendling asynchronous reads: %d.  \r\n",m_iNumberOfPendlingReads);
		AppendLog(msg);
		...
		if ( bRet )
		{
			msg.Format("Successfully started %i Input Output Worker thread(s).  \r\n",m_nIOWorkers );
			AppendLog(msg);
		}
	...
	if ( bRet )
	{
		if ( m_nPortNumber>0 )
		{
			...
			AppendLog(msg);
			msg.Format("Waiting for clients on adress: %s, port:%i.  \r\n",GetHostIP(),m_nPortNumber);
			AppendLog(msg);	
		}
		else 
		{
...
UINT IOCPS::ListnerThreadProc(LPVOID pParam)
{
	...
	if ( pThis )
	{
		while ( !pThis->m_bShutDown )
		{
			...
			// Wait for multiple events wait timeout.
				...
				if ( events.iErrorCode[FD_ACCEPT_BIT] == 0&&pThis->m_bAcceptConnections&&!pThis->m_bShutDown)
				{
					...
					if (clientSocket == SOCKET_ERROR)
					{
						...
						if (nRet != WSAEWOULDBLOCK)
						{
							...
							//
							CStringA msg;
							msg.Format("Wsaaccept() error: %s.  \r\n",pThis->ErrorCode2Text(WSAGetLastError()));
							pThis->AppendLog(msg);
						}
					}else
...
UINT IOCPS::IOWorkerThreadProc(LPVOID pParam)
{
	...
	if ( pThis )
	{
		...
		while ( !bError && !pThis->m_bShutDown)
		{
			pOverlapBuff=NULL;
...
{	
	...
	if(pContext!=NULL)
	{
		...
		*/
		...
		{	
			...
			{
				CStringA msg;
				msg.Format("AssociateSocketWithCompletionPort Failed: %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
				AppendLog(msg);
			...
			{
				CStringA msg;
				msg.Format("ERROR Could not allocate memory for buffer in Acceptincoming: %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
				AppendLog(msg);
				DisconnectClient(pContext);
				ReleaseClientContext(pContext);
				return FALSE;
		...
		{
			CStringA msg;
			msg.Format("ERROR HASHMAP AddClientContext failed.  %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
			AppendLog(msg);
			return FALSE;
		}
	}else
	{
		CStringA msg;
		msg.Format("ERROR Could not allocate memory for Context in Acceptincoming: %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
		AppendLog(msg);
		closesocket(clientSocket);
		return FALSE;
	}
...
BOOL IOCPS::AddClientContext(ClientContext *mp)
{
	...
	// Set the current ClientContext object.
	SetCurrentClientContext(mp);
	...
	if(pContext&&!m_bShutDown)
	{
		if(pOverlapBuff==NULL)
		{
			...
			if(pOverlapBuff==NULL)
			{
				...
				CStringA msg;
				msg.Format("FAILED TO Allocate buffer memory in OnRead %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
				AppendLog(msg);
				DisconnectClient(pContext);
				ReleaseClientContext(pContext);
				return;
			...
			else
			{
				...
				if ( nRetVal == SOCKET_ERROR && WSAGetLastError() != WSA_IO_PENDING) 
				{
					if(WSAGetLastError()!=WSAENOTSOCK)
					{
						...
						{
							CStringA msg;
							msg.Format("Disconnect in Onread Possible Socket Error: %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
							AppendLog(msg);
						}
					}
					ReleaseBuffer(pOverlapBuff);
...
void IOCPS::OnPostedPackage(ClientContext *pContext, CIOCPBuffer *pOverlapBuff)
{
	...
	{
		UINT nSize = 0;
		memmove(&nSize,pOverlapBuff->GetBuffer(),MINIMUM_PACKAGE_SIZE);
		pContext->m_ContextLock.Lock();
...
void IOCPS::OnReadCompleted(ClientContext *pContext, DWORD dwIoSize,CIOCPBuffer *pOverlapBuff)
{
	...
	{
		CStringA msg;
		msg.Format("Disconnection in OnReadCompleted (dwIoSize == 0||pOverlapBuff==NULL) errcode: %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
		AppendLog(msg);
	...
	{
		//
		// Process The package assuming that it have a heap of size (MINIMUM_PACKAGE_SIZE) 
...
/*
* Assumes that Packages arrive with A MINIMUM_PACKAGE_SIZE header and builds Packages that  
...
void IOCPS::ProcessPackage(ClientContext *pContext, DWORD dwIoSize, CIOCPBuffer *pOverlapBuff)
{
	...
	if ( pBuffPartialMessage!=NULL )
	{
		...
		if ( nUsedBuffer < MINIMUM_PACKAGE_SIZE )
		{
			//Header to small.. 
			UINT nHowMuchIsNeeded=MINIMUM_PACKAGE_SIZE-nUsedBuffer;
		...
		if ( nUsedBuffer >= MINIMUM_PACKAGE_SIZE )
		{
			...
			memmove(&nSize,pBuffPartialMessage->GetBuffer(),MINIMUM_PACKAGE_SIZE);
			// The Overlapped Package is good. Never send packages bigger that the MAXIMUM_PACKAGE_SIZE-MINIMUM_PACKAGE_SIZE
			if ( nSize<=(MAXIMUM_PACKAGE_SIZE-MINIMUM_PACKAGE_SIZE) )
			{
				nHowMuchIsNeeded=nSize-(nUsedBuffer-MINIMUM_PACKAGE_SIZE);
			...
			{
				CStringA msg;
				msg.Format("Package size error > MAXIMUM_PACKAGE_SIZE-MINIMUM_PACKAGE_SIZE: %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
				AppendLog(msg);
				ReleaseBuffer(pOverlapBuff);
				pOverlapBuff=NULL;
				ReleaseBuffer(pContext->m_pBuffOverlappedPackage);
	...
	do 
	{
		...
		if ( nUsedBuffer >= MINIMUM_PACKAGE_SIZE )
		{
			...
			if ( nSize==nUsedBuffer-MINIMUM_PACKAGE_SIZE )
			{
				NotifyReceivedPackage(pOverlapBuff,nSize,pContext);
				pOverlapBuff->EmptyUsed();
				done = true;
			}else if ( nUsedBuffer-MINIMUM_PACKAGE_SIZE >nSize )
			{
				// We have more data 
				CIOCPBuffer *pBuff=SplitBuffer(pOverlapBuff,nSize+MINIMUM_PACKAGE_SIZE);
				NotifyReceivedPackage(pBuff,nSize,pContext);
				ReleaseBuffer(pBuff);
				// loop again, we may have another complete message in there...
				done = false;
			}else if ( nUsedBuffer-MINIMUM_PACKAGE_SIZE<nSize && nSize<MAXIMUM_PACKAGE_SIZE)
			{
				//
			...
			}else if ( nSize>MAXIMUM_PACKAGE_SIZE )
			{			
				//pOverlapBuff->DUMP();
				CStringA msg;
				msg.Format("Messagesize error: %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
				AppendLog(msg);
#ifdef SIMPLE_SECURITY
				AddToBanList(pContext->m_Socket);
#endif
		...
		{   //  Header  too small. 
			// nUsedBuffer < MINIMUM_PACKAGE_SIZE
...
void IOCPS::OnWrite(ClientContext *pContext, DWORD dwIoSize,CIOCPBuffer *pOverlapBuff)
{
	...
	{	
		...
		while(pOverlapBuff!=NULL)
		{
			...
			* when an send is not ordered and 
	...
	if(pContext!=NULL)
	{
		...
		if (pOverlapBuff!=NULL)
		{
			if(pOverlapBuff->GetUsed()!=dwIoSize)
			{
				...
				CStringA msg;
				msg.Format("The whole message was not Sent.: %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
				ReleaseBuffer(pOverlapBuff);
				AppendLog(msg);
			}else
			{
	...
	while (pos != NULL) 
	{
		...
		if(pContext!=NULL)
		{
			...
			if(pOverlapBuff!=NULL)
			{
				...
				{
					CStringA msg;
					msg.Format("Buffer Error in  ASendToAll, AddData Failed: %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
					AppendLog(msg);
					ReleaseBuffer(pOverlapBuff);
					bRet&=FALSE;
					break;	
			...
			{
				CStringA msg;
				msg.Format("Could not allocate memory ASendToAll: %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
				AppendLog(msg);
				bRet=FALSE;
				break;
			}
...
CStringA IOCPS::GetHostKeyName(SOCKET socket)
{
	sockaddr_in sockAddr;
...
LPTSTR IOCPS::GetProfileValue(LPCTSTR szSection, LPCTSTR szName, LPCTSTR szFileName, LPCTSTR szDefualt /*= _T("")*/)
{
	const int len_buffer = 1024;
	TCHAR* buffer = _T("");
	CString strFileFullPath = GetCurretAppPath();
	strFileFullPath += szFileName;
	GetProfileValueFromIniFile(szSection, szName, strFileFullPath, &buffer, len_buffer, szDefualt);
	return (LPTSTR)buffer;
}

int IOCPS::GetProfileValueFromIniFile(LPCTSTR szSection, LPCTSTR szName, LPCTSTR szFileName, TCHAR** pBuffer, int nBufferLen, LPCTSTR szDefualt)
{
...
CString IOCPS::GetCurretAppPath()
{
	CString strAppPath;
	if(strAppPath.GetLength() < 1)
	{
		...
		{
			TCHAR* pLastQu = _tcsrchr(szAppBuffer, _T('\\'));
			if(pLastQu)
			{
				*(pLastQu + 1) = _T('\0');
...
CStringA IOCPS::GetHostAddress(SOCKET socket)
{
	sockaddr_in sockAddr;
	memset(&sockAddr, 0, sizeof(sockAddr));
	...
	if(pContext)
	{
		...
		if(pOverlapBuff==NULL)
		{
			...
			if(pOverlapBuff==NULL)
			{
				...
				CStringA msg;
				msg.Format("FAILED TO Allocate buffer memory in OnZeroByteRead: %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
				AppendLog(msg);
				DisconnectClient(pContext);
				ReleaseClientContext(pContext);
				return; 
		...
		if ( nRetVal == SOCKET_ERROR && WSAGetLastError() != WSA_IO_PENDING) 
		{
			if(WSAGetLastError()!=WSAENOTSOCK)
			{
				...
				{
					CStringA msg;
					msg.Format("Disconnect in ZeroByteRead Possible Socket Error: %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
					AppendLog(msg);
				}
			}
			ReleaseBuffer(pOverlapBuff);
...
BOOL IOCPS::AZeroByteRead(ClientContext *pContext, CIOCPBuffer *pOverlapBuff)
{
	...
	if(pContext->m_Socket!=INVALID_SOCKET )
	{
		...
		{            
			...
			ReleaseClientContext(pContext);
			CStringA msg;
			msg.Format("Disconnect in AZeroByteRead Possible Socket Error: %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
			AppendLog(msg);
			return FALSE;
		}
...
CStringA IOCPS::ErrorCode2Text(DWORD dw)
{
	CStringA error="";
...
ClientContext* IOCPS::AllocateContext()
{
	...
	pContext->m_ID=0;
	pContext->m_Socket=INVALID_SOCKET;
	pContext->m_nNumberOfPendlingIO=1;
	...
	pContext->m_pBuffOverlappedPackage=NULL;
	pContext->m_strClientLocation = _T("");
...
// Set current ClientContext.
void IOCPS::SetCurrentClientContext(ClientContext* clientContext)
{
	m_clientContext = clientContext;
}

// Get current ClientContext.
ClientContext* IOCPS::GetCurrentClientContext()
{
...
CIOCPBuffer *IOCPS::AllocateBuffer(int nType)
{
	...
	if(pBuff==NULL)
	{
		...
		{ 
			...
			CStringA msg;
			msg.Format("Error new CIOCPBuffer(); failed in AllocateBuffer %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
			AppendLog(msg);
			return NULL;	
		}	
	}
	...
	if(pBuff!=NULL) 
	{
		...
		{	
			CStringA msg;
			msg.Format(" Error m_BufferList.AddHead((void*)pBuff) failed in AllocateBuffer: %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
			AppendLog(msg);
			delete pBuff;
			m_BufferListLock.Unlock();
			return NULL;
...
BOOL IOCPS::ReleaseBuffer(CIOCPBuffer *pBuff)
{
	...
	if(pos==NULL)
	{
		...
		CStringA msg;
		msg.Format("FATAL ERROR pBuff->GetPosition(); returned NULL in ReleaseBuffer: %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
		AppendLog(msg);
		m_BufferListLock.Unlock();
		return FALSE;
	}
...
BOOL IOCPS::SetupListner()
{
	...
	if ( m_socListen == INVALID_SOCKET )
	{
		...
		CStringA msg;
		msg.Format("Could not create listen socket: %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
		AppendLog(msg);
		return FALSE;
	}
	...
	{	
		CStringA msg;
		msg.Format("WSACreateEvent() error : %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
		AppendLog(msg);
		closesocket(m_socListen);
		return FALSE;
	}
	...
	if ( nRet == SOCKET_ERROR )
	{
		...
		CStringA msg;
		msg.Format("WSAAsyncSelect() error: %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
		AppendLog(msg);
		closesocket(m_socListen);
		return FALSE;
	}
	...
	if ( !m_pListenThread )
	{
		...
		CStringA msg;
		msg.Format("Error Could not start the ListnerThreadProc: %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
		AppendLog(msg);
		return FALSE;
	}
	return TRUE; 
...
BOOL IOCPS::CreateCompletionPort()
{
	...
	{	
		CStringA msg;
		msg.Format("Error Could not start Create Socket in CreateCompletionPort: %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
		AppendLog(msg);
		return FALSE;
	}
	...
	{	
		CStringA msg;
		msg.Format("CreateIoCompletionPort Failed: %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
		AppendLog(msg);
		closesocket( s );
		return FALSE;
	}
...
BOOL IOCPS::SetupIOWorkers()
{
	...
	for(int i=0; i<m_iMaxIOWorkers;i++)
	{
		...
		else
		{
			...
			CStringA msg;
			msg.Format("Error Couldnot start worker: %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
			AppendLog(msg);
...
*
*/
BOOL IOCPS::Connect(const CStringA &strIPAddr, int nPort)
{
	...
	if (clientSocket == INVALID_SOCKET)
	{
		static unsigned int nCreateSockCount = 5;
		if (nCreateSockCount > 0)
		{
			CStringA msg;
//			msg.Format("Connect()->socket() Could not create  socket: %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
			msg.Format("Connect()->socket() Could not create  socket: %d.  \r\n", WSAGetLastError());
			AppendLog(msg);
			return FALSE;
			nCreateSockCount--;
		}
	...
	{
		static unsigned int nConnectCount = 10;
		if (nConnectCount > 0)
		{
			CStringA msg;
//			msg.Format("Connection Error: %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
			msg.Format("Connection Error: %d.  \r\n", WSAGetLastError());
			AppendLog(msg);
			nConnectCount--;
		}
		closesocket(clientSocket);
...
void IOCPS::SetSendSequenceNumber(ClientContext *pContext,CIOCPBuffer *pBuff)
{
	if (pContext!=NULL&&pBuff!=NULL)
	{
		...
		pContext->m_SendSequenceNumber=(pContext->m_SendSequenceNumber+1)%MAXIMUM_SEQUENSE_NUMBER;
...
void IOCPS::MakeOrderdRead(ClientContext *pContext, CIOCPBuffer *pBuff)
{
	if (pContext!=NULL&&pBuff!=NULL)
	{
		...
		{
			pContext->m_ReadSequenceNumber=(pContext->m_ReadSequenceNumber+1)%MAXIMUM_SEQUENSE_NUMBER;
			//pContext->m_ContextLock.Unlock();
		}
	}
}
...
// 			return FALSE;
		...
		{
			CStringA msg;
			msg.Format("Could not allocate memory in PrepareSendFile: %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
			AppendLog(msg);
			DisconnectClient(pContext);
			return FALSE;
		}
...
void IOCPS::IncreaseReadSequenceNumber(ClientContext *pContext)
{
	if (pContext!=NULL)
	{
		pContext->m_ContextLock.Lock();
		// increase or reset the sequence number 
		pContext->m_CurrentReadSequenceNumber=(pContext->m_CurrentReadSequenceNumber+1)%MAXIMUM_SEQUENSE_NUMBER;
		pContext->m_ContextLock.Unlock();
	}
}
...
void IOCPS::IncreaseSendSequenceNumber(ClientContext *pContext)
{
	if (pContext!=NULL)
	{
		pContext->m_ContextLock.Lock();
		// increase or reset the sequence number 
		pContext->m_CurrentSendSequenceNumber=(pContext->m_CurrentSendSequenceNumber+1)%MAXIMUM_SEQUENSE_NUMBER;
		pContext->m_ContextLock.Unlock();
	}
}
...
void IOCPS::AppendLog(CStringA msg)
{
}


// Set the mode of transmit file.
void IOCPS::SetFileTransmitMode(bool bIsFileTransmit)
{
	EnterCriticalSection(&m_criFileTransmit);
	m_bIsFileTransmit = bIsFileTransmit;
	LeaveCriticalSection(&m_criFileTransmit);
}

// Is the mode of transmit file?
bool IOCPS::IsFileTransmitMode()
{
	EnterCriticalSection(&m_criFileTransmit);
	bool bIsFileTransmit = m_bIsFileTransmit;
	LeaveCriticalSection(&m_criFileTransmit);
	return bIsFileTransmit;
}
...
void IOCPS::NotifyFileCompleted(ClientContext *pcontext)
{
	...
	SetEvent(m_FileTransmitSuccHandle);
#endif
}
	...
	{	
		...
		if(pOverlapBuff!=NULL)
		{
			...
			if ( nRetVal == SOCKET_ERROR && WSAGetLastError() != WSA_IO_PENDING ) 
			{
				...
				{
					CStringA msg;
					msg.Format("Error in On TransmitFile..: %s.  \r\n",ErrorCode2Text(WSAGetLastError()));
					AppendLog(msg);
				}
				ReleaseBuffer(pOverlapBuff);
				//DisconnectClient(pContext);
...
*  #DEFINE SIMPLE_SECURITY to block multiple connections from the
*  same IP.  
*/
...
*
*	This function is used with ConnectAcceptCondition and
*  #DEFINE SIMPLE_SECURITY to block connections that behave badly. 
*  
...
void IOCPS::AddToBanList(SOCKET &Socket)
{
	...
	CString msg="";
	msg.Format("%s banned because of bad messagesize.  \r\n",sIP);
	AppendLog(msg);
	*/
	//
	//  Get the incoming connection IPadress
...
{
	CStringA sKey_PATH="\\SYSTEM\\CurrentControlSet\\Services\\Tcpip\\Parameters";
	CStringA sKey_SynAttackProtect="SynAttackProtect";
	CStringA sKey_TcpMaxHalfOpen="TcpMaxHalfOpen";
	CStringA sKey_TcpMaxHalfOpenRetried="TcpMaxHalfOpenRetried";
	CStringA sKey_TcpMaxPortsExhausted="TcpMaxPortsExhausted";
	CStringA sKey_TcpMaxConnectResponseRetransmissions="TcpMaxConnectResponseRetransmissions";
	...
	//
	val=iValue;
	if (RegOpenKeyA(HKEY_LOCAL_MACHINE, sKey_PATH, &hKey) != ERROR_SUCCESS)
		if (RegCreateKeyA(HKEY_LOCAL_MACHINE, sKey_SynAttackProtect, &hKey) != ERROR_SUCCESS)
			return FALSE;
	r = RegSetValueExA(hKey, sKey_SynAttackProtect, 0, REG_DWORD, (BYTE *)&val, sizeof(val));
	RegCloseKey(hKey);
	...
	if(iValue==1)
	{
		...
		if (RegOpenKeyA(HKEY_LOCAL_MACHINE, sKey_PATH, &hKey) != ERROR_SUCCESS)
			if (RegCreateKeyA(HKEY_LOCAL_MACHINE, sKey_TcpMaxHalfOpen, &hKey) != ERROR_SUCCESS)
				return FALSE;
		r = RegSetValueExA(hKey, sKey_TcpMaxHalfOpen, 0, REG_DWORD, (BYTE *)&val, sizeof(val));
		RegCloseKey(hKey);
		bRet&= (r == ERROR_SUCCESS);
		...
		val=iTcpMaxHalfOpenRetried;
		if (RegOpenKeyA(HKEY_LOCAL_MACHINE, sKey_PATH, &hKey) != ERROR_SUCCESS)
			if (RegCreateKeyA(HKEY_LOCAL_MACHINE, sKey_TcpMaxHalfOpenRetried, &hKey) != ERROR_SUCCESS)
				return FALSE;
		r = RegSetValueExA(hKey, sKey_TcpMaxHalfOpenRetried, 0, REG_DWORD, (BYTE *)&val, sizeof(val));
		RegCloseKey(hKey);
		bRet&= (r == ERROR_SUCCESS);
		...
		if (RegOpenKeyA(HKEY_LOCAL_MACHINE, sKey_PATH, &hKey) != ERROR_SUCCESS)
			if (RegCreateKeyA(HKEY_LOCAL_MACHINE, sKey_TcpMaxPortsExhausted, &hKey) != ERROR_SUCCESS)
				return FALSE;
		r = RegSetValueExA(hKey, sKey_TcpMaxPortsExhausted, 0, REG_DWORD, (BYTE *)&val, sizeof(val));
		RegCloseKey(hKey);
		bRet&= (r == ERROR_SUCCESS);
		...
		if (RegOpenKeyA(HKEY_LOCAL_MACHINE, sKey_PATH, &hKey) != ERROR_SUCCESS)
			if (RegCreateKeyA(HKEY_LOCAL_MACHINE, sKey_TcpMaxConnectResponseRetransmissions, &hKey) != ERROR_SUCCESS)
				return FALSE;
		r = RegSetValueExA(hKey, sKey_TcpMaxConnectResponseRetransmissions, 0, REG_DWORD, (BYTE *)&val, sizeof(val));
		RegCloseKey(hKey);
		bRet&= (r == ERROR_SUCCESS);
