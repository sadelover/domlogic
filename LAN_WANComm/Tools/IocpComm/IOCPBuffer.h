...
public:
	...
	BOOL GetPackageInfo(BYTE &Type, BYTE &key, CStringA &stxt);
	BOOL CreatePackage(BYTE Type, BYTE key,  CStringA stxt);
	BOOL GetPackageInfo(BYTE &Type,BYTE &key1,BYTE &key2,BYTE &key3, CStringA &stxt);
	...
	BOOL CreatePackage(CStringA stxt);
	BOOL CreatePackage(BYTE Type,CStringA stxt);
	BOOL CreatePackage(BYTE Type,UINT nData, CStringA stxt);
	BOOL CreatePackage(BYTE Type);
    BOOL CreatePackage(BYTE Type,BYTE key1,BYTE key2,BYTE key3, CStringA stxt);
	//
...
private:
	...
	UINT    m_nUsed;
	// The buffer. 
	BYTE m_Buffer[MAXIMUM_PACKAGE_SIZE];
